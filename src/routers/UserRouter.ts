...
export class UserRouter {
   ...
   */
  private _init() {
    this.router.get('/', auth, (req, res, next) => this.getAll(<AuthenticatedRequest>req, res, next));
    ...
    this.router.get('/whoami', auth, (req, res, next) => this.whoAmI(<AuthenticatedRequest>req, res, next));
  }

  public getAll(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    this.repository.getAll()
      .then(users => res.send(users))
      .catch(err => next(err));
  }

  public whoAmI(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    res.json({
      status: 'success',
      message: 'here\'s who',
      user: req.user,
